...
class Constfuncs:
    ...
    def prepare_parent_nfts(self):
        ...
        if self.breeding_type == 'Heterogeneous':
            if self.args.module_id == 1:
                parent_nft_candidate = torch.stack([self.Vj.topk(self.args.cand_lim)[1] for __ in range(self.nftP.N)]).to(self.args.device)
            else:
                # self.args.module_id == 2: random
                mask = (torch.rand(self.nftP.N, self.args.cand_lim) > 0.5).long().to(self.args.device) if self.args.module_id == 2 else 1
                parent_nft_candidate = []
                for batch_user in make_batch_indexes(self.nftP.N, 1024):
                    parent_nft_candidate.append( 
                        mask* (self.Uij[batch_user] * self.Vj).topk(self.args.cand_lim)[1] + (1-mask) * (self.Uij[batch_user] * self.Vj).topk(self.args.cand_lim, largest=False)[1]
                        ) 
                parent_nft_candidate = torch.cat(parent_nft_candidate)    
            ...
            if self.args.module_id == 0:
                ...
                parent_nft_candidate = []
                for batch_user in make_batch_indexes(self.nftP.N, 1024):
                    parent_nft_candidate.append((self.Uij[batch_user] * self.Vj*population_factor).topk(self.args.cand_lim)[1])
                parent_nft_candidate = torch.cat(parent_nft_candidate)    
            ...
            iterator = make_batch_indexes(parent_nft_candidate.size(0), chunk_size)
            total = parent_nft_candidate.size(0)// chunk_size + 1
            pbar = tqdm(iterator, ncols=88, total=total)
            for batch_buyer_idx in pbar:
