...
class AuctionSolver(BaseSolver):
    ...
    def run_bidding(self):
        ...
        self.pricing = (1 + torch.rand(self.nftP.M, device=self.args.device))* sum(self.buyer_budgets)/self.nftP.M
        ...
        eps = 0.01
        for iter_ in tqdm(range(64), ncols=88, desc='auction process'):
